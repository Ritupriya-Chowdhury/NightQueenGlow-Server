  ...
  try {
    ...
    app.get('/products/:id', async (req, res) => {
      try {
        ...
        res.status(500).send({ error: true, message: 'Failed to fetch product' });
      }
    });
...
app.post('/wishlist/:productId', verifyJWT, verifyBuyer, async (req, res) => {
  try {
    ...
    if (result.modifiedCount === 0) {
      return res.status(400).send({ error: true, message: 'Failed to add to wishlist' });
    }

    res.send({ success: true, message: 'Product added to wishlist successfully' });
  } catch (error) {
    res.status(500).send({ error: true, message: 'Failed to add to wishlist' });
  }
});


// Remove from Wishlist (Buyer only)
app.delete('/wishlist/:productId', verifyJWT, verifyBuyer, async (req, res) => {
  try {
    const email = req.decoded.email; // Extract email from JWT
    const productId = req.params.productId;
    ...
    if (result.modifiedCount === 0) {
      return res.status(400).send({ error: true, message: 'Failed to remove from wishlist' });
    }

    res.send({ success: true, message: 'Product removed from wishlist successfully' });
  } catch (error) {
    res.status(500).send({ error: true, message: 'Failed to remove from wishlist' });
  }
});
...
app.post('/cart/:productId',verifyJWT,verifyBuyer,  async (req, res) => {
  try {
    ...
    if (product.quantity < 1) {
      return res.status(400).send({ error: true, message: 'Product out of stock' });
    }
    ...
    console.error(error);
    res.status(500).send({ error: true, message: 'Failed to add product to cart' });
  }
});
// Get Cart (Buyer only)
app.get('/cart', verifyJWT, verifyBuyer, async (req, res) => {
  try {
    ...
    app.post('/jwt', (req, res) => {
      ...
      const token = jwt.sign({email: user.email}, JWT_ACCESS_SECRET, { expiresIn: '1h' });
      res.send({ token });
    });
